...
class ChatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    @objc func keyBoardWillShow(notification: Notification){
        if let userInfo = notification.userInfo as? Dictionary<String, AnyObject>{
            ...
            heightConstraint.constant = keyboardHeight + 30
        }
    }
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        UIView.animate(withDuration: 0.5) {
            self.heightConstraint.constant = self.keyboardHeight + 30
    ...
    @IBAction func sendPressed(_ sender: AnyObject) {
        ...
        let messDic = ["sender": Auth.auth().currentUser?.email, "MessBody": messageTextfield.text!]
        
        messDB.childByAutoId().setValue(messDic) {
            (error, reference) in
    ...
    func retrieveMessages(){
        ...
        messDB.observe(.childAdded) { (snapshot) in
            let snapshotValue = snapshot.value as! Dictionary<String, String>
            
            let message = Message()
            let text = snapshotValue["MessBody"]!
